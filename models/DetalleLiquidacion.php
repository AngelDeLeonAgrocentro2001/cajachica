...
class DetalleLiquidacion {
        ...
        try {
            $stmt = $this->pdo->prepare("
                INSERT INTO detalle_liquidaciones (
                    ...
                    p_unitario, total_factura, estado, id_centro_costo, cantidad, serie, rutas_archivos
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            return $stmt->execute([
                $id_liquidacion, $tipo_documento, $no_factura, $nombre_proveedor, $nit_proveedor, $dpi, $fecha, $t_gasto,
    ...
    public function updateDetalleLiquidacion($id, $id_liquidacion, $tipo_documento, $no_factura, $nombre_proveedor, $nit_proveedor, $dpi, $fecha, $t_gasto, $p_unitario, $total_factura, $estado, $id_centro_costo, $cantidad = null, $serie = null, $rutas_archivos = '[]') {
        try {
            $stmt = $this->pdo->prepare("
                ...
                SET id_liquidacion = ?, tipo_documento = ?, no_factura = ?, nombre_proveedor = ?, nit_proveedor = ?, dpi = ?, 
                    fecha = ?, t_gasto = ?, p_unitario = ?, total_factura = ?, estado = ?, id_centro_costo = ?, 
                    cantidad = ?, serie = ?, rutas_archivos = ?, updated_at = NOW()
                WHERE id = ?
            ");
            return $stmt->execute([
                $id_liquidacion, $tipo_documento, $no_factura, $nombre_proveedor, $nit_proveedor, $dpi, $fecha, $t_gasto,
                $p_unitario, $total_factura, $estado, $id_centro_costo, $cantidad, $serie, $rutas_archivos, $id
            ]);
        } catch (PDOException $e) {
    ...
    public function getDetallesByLiquidacionId($id_liquidacion) {
        $stmt = $this->pdo->prepare("
            SELECT dl.*, cc.nombre AS nombre_centro_costo 
